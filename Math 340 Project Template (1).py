#use these five global variables to set the directories for your data and output files for each
...
def test_halls(priorities_filename,boy_set_label,girl_set_label):
    ...
    boy_set_label = read_priorities(priorities_filename)['B']
...
def find_rogues(pairs_filename, priorities_filename):
    ...
    if 'R0' in pairs:
        for girl in pairs:
            husband = pairs[girl]
            for boy in priorities["B"]:
                index = list((pairs.values())).index(boy)
                wife = list(pairs.keys())[index]

                if (priorities['B'][boy].index(wife) > priorities['B'][boy].index(girl) \
                    and priorities['R'][girl].index(husband) > priorities['R'][girl].index(boy)):
                    rogue_pairs.add((boy, girl))
                    
    
    else:
        for boy in pairs:
            wife = pairs[boy]
            for girl in priorities["R"]:
                index = list((pairs.values())).index(girl)
                husband = list(pairs.keys())[index]

                if (priorities['R'][girl].index(husband) > priorities['R'][girl].index(boy) \
                    and priorities['B'][boy].index(wife) > priorities['B'][boy].index(girl)):
                    rogue_pairs.add((boy, girl))
...
def pair(csv_path,boy_set_label,girl_set_label):
    ...
    priorities = read_priorities(csv_path)
    boys = priorities[boy_set_label] # dictionary all boys and their priorities
    girls = priorities[girl_set_label] # dictionary all girls and their priorities
    free_boys = list(boys.keys()) # set of all the boys
    pairs = {} # dictionary that will include each stable pairing

    while free_boys: # while there are still boys to get engaged
        for current_boy in free_boys:
            current_girl = boys[current_boy][0] # gets the top girl
            
            if current_girl not in pairs.values(): # if the current girl is not paired up
                pairs[current_boy] = current_girl # add key value pair for current boy

            else: # if the current girl is paired up then make sure its optimal
                current_partner = list(pairs.keys())[list(pairs.values()).index(current_girl)] # finds spouse
                if girls[current_girl].index(current_boy) < girls[current_girl].index(current_partner):
                    del pairs[current_partner] # deletes old pairing
                    free_boys.append(current_partner) # adds boy back to free boys
                    pairs[current_boy] = current_girl # adds new optimal pairing
                    boys[current_partner].remove(current_girl)
                else:
                    boys[current_boy].remove(current_girl)
                    free_boys.append(current_boy) # adds boy back to free boys
...
#     import os
#     print(os.getcwd())
#     path="/Users/lucasowen/MATH340/Group Project/example input-output/"
#     rogues=find_rogues(path+"T2example/data/size_6_pairings_1.csv",path+"T2example/data/size_6_priorities.csv")
#     #rogues=find_rogues(T2_data_path+"size_"+str(size)+"_pairings_"+str(pairing)+".csv", T2_data_path+"size_"+str(size)+"_priorities.csv")
#     of=open(T2_soln_path+"size_"+str(size)+"_rogues_"+str(pairing)+".txt","w")
#     of.write(str(rogues))
#     of.close()
